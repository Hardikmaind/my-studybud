...
def room(request, pk):
    ...
    if request.method == "POST":
        message = Message.objects.create(
            user=request.user,
            room=room,
            # here in get(' ')  i have put the name of the input there in this case it is body
        ...
        # here we are adding the user to the room participants list...jo bhi naya user chatt karega room mein usko particiipants ke list mein add kiya...yaha pe request.user is the user who is sending the message
...
def userProfile(request, pk):
    ...
    # we can get all the children of the specific object by setting the .._set.all() method
...
def createRoom(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():  
            ...
            # below we have set the host of the room as the user which is logged in ...which is present inside the request object
